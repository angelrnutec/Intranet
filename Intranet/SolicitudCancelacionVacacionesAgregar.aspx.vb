...
Public Class SolicitudCancelacionVacacionesAgregar
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Request.QueryString("id") = "0" Then
            CrearControlesDinamicos(Me.ddlSolicitudesOriginales.SelectedValue)
        End If
    ...
    Private Sub CargaCombos()
        Try
            ...
            Dim solicitud_vacacion As New SolicitudCancelacionVacacion(System.Configuration.ConfigurationManager.AppSettings("CONEXION"))
            Me.ddlSolicitudesOriginales.Items.Clear()
            Me.ddlSolicitudesOriginales.Items.AddRange(Funciones.DatatableToList(solicitud_vacacion.RecuperaSolicitudVacacionesParaCancelar(Session("idEmpleado")), "descripcion", "id_solicitud"))
            Me.ddlSolicitudesOriginales.SelectedValue = Session("idEmpresa")
            Me.ddlSolicitudesOriginales.Enabled = True
    ...
    Private Sub CargaDatos(id As Integer)
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                End If
                Me.btnGuardar.Visible = False
                Me.trSolicitudOriginal.Visible = False

                Dim sbTexto As New StringBuilder("")
                sbTexto.Append("<br>" & TranslateLocale.text("Folio original:") & dr("folio_txt_original") & "<ul>")
                For Each Str As String In Split(dr("fechas"), "|")
                    If Str.Length > 0 Then
                        sbTexto.Append("<li>" & Convert.ToDateTime(Str).ToString("dd/MM/yyyy") & "</li>")
                    End If
                Next
                sbTexto.Append("</ul>")

                Me.phFechasCancelar.Controls.Add(New LiteralControl(sbTexto.ToString()))
    ...
    '    Dim dt As DataTable = asueto.RecuperaFuturos()
    ...
    Private Sub btnRegresar_Click(sender As Object, e As EventArgs) Handles btnRegresar.Click
        Response.Redirect("SolicitudCancelacionVacaciones.aspx")
    End Sub
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        Try
            ...
            Dim fechas_a_cancelar As String = ""
            Dim dias_a_cancelar As String = ""

            For Each ctrl As Control In phFechasCancelar.Controls
                For Each ctrl2 As Control In ctrl.Controls
                    For Each ctrl3 As Control In ctrl2.Controls
                        If TypeOf ctrl3 Is CheckBox Then
                            If CType(ctrl3, CheckBox).Checked = True Then
                                Dim idTexto = Replace(CType(ctrl3, CheckBox).ID, "chkDiasCancelar_", "")
                                fechas_a_cancelar &= Split(idTexto, "_")(0) & "|"
                                dias_a_cancelar &= (Convert.ToInt32(Split(idTexto, "_")(1)) / 10).ToString() & "|"
                            End If
                        End If
                    Next
                Next
            ...
            Dim dt As DataTable = solicitud_vacacion.Guarda(Me.txtIdSolicitud.Text, Me.ddlSolicitante.SelectedValue,
                                   Me.ddlAutorizaJefe.SelectedValue, Me.ddlNomina.SelectedValue, fechas_a_cancelar, dias_a_cancelar,
                                   Me.ddlSolicitudesOriginales.SelectedValue, Me.txtComentarios.Text,
                                   Me.ddlEmpresa.SelectedValue, Session("idEmpleado"))
    ...
    Private Sub ValidaCaptura()
        ...
        If Me.ddlSolicitudesOriginales.SelectedValue = 0 Then
            msg += " - " & TranslateLocale.text("Solicitud Original") & "\n"
        Else
            Dim i As Integer = 0
            For Each ctrl As Control In phFechasCancelar.Controls
                For Each ctrl2 As Control In ctrl.Controls
            ...
            Next
            If i = 0 Then
                msg += " - " & TranslateLocale.text("Fechas a cancelar") & "\n"
            End If
        End If
    ...
    Private Sub EmailJefeDirecto()
        ...
        Try
            ...
            Dim solicitud_vacacion As New SolicitudCancelacionVacacion(System.Configuration.ConfigurationManager.AppSettings("CONEXION").ToString())
            ...
            If dt.Rows.Count > 0 Then
                ...
                email_body = RetrieveHttpContent(email_url_base_local & "/Email_Formatos/EnvioSolicitudCancelacionVacaciones.aspx?id=" & Me.txtIdSolicitud.Text, errorMsg)
                ...
                Try
                    ...
                    seguridad.GuardaLogDatos("EnvioSolicitudCancelacionVacaciones: " & resultado & "::::" & email_para & "---->" & email_url_base & "/Email_Formatos/EnvioSolicitudCancelacionVacaciones.aspx?id=" & Me.txtIdSolicitud.Text)
                Catch ex As Exception
    ...
    Protected Sub ActualizaAutorizaJefe(id_empleado_incluir As Integer)
        ...
        'Dim sv As New SolicitudCancelacionVacacion(System.Configuration.ConfigurationManager.AppSettings("CONEXION"))
    ...
    Private Sub btnCancelarSolicitud_Click(sender As Object, e As EventArgs) Handles btnCancelarSolicitud.Click
        Try
            Dim solicitud As New SolicitudCancelacionVacacion(System.Configuration.ConfigurationManager.AppSettings("CONEXION"))
            solicitud.CancelaSolicitud(Me.txtIdSolicitud.Text, Session("idEmpleado"))

            Response.Redirect("SolicitudCancelacionVacacionesAgregar.aspx?id=" & Me.txtIdSolicitud.Text)
        Catch ex As Exception
            ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType, "script", "<script>alert('" & ex.Message.Replace(ChrW(39), ChrW(34)) & "');</script>", False)
        End Try
    End Sub

    Private Sub ddlSolicitudesOriginales_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlSolicitudesOriginales.SelectedIndexChanged
        'CrearControlesDinamicos(Me.ddlSolicitudesOriginales.SelectedValue)
    End Sub


    Private Sub CrearControlesDinamicos(id_solicitud As Integer)
        Try
            Dim solicitud As New SolicitudCancelacionVacacion(System.Configuration.ConfigurationManager.AppSettings("CONEXION"))
            Dim dt As DataTable = solicitud.RecuperaSolicitudVacacionesDiasParaCancelar(id_solicitud)

            If dt.Rows.Count > 0 Then
                Dim ul As New HtmlGenericControl("ul")
                ul.ID = "ulDiasCancelar"
                For Each dr As DataRow In dt.Rows

                    Dim fecha As DateTime = dr("fecha")
                    Dim dias As Decimal = dr("dias")

                    Dim li As New HtmlGenericControl("li")
                    Dim checkbox As New CheckBox()
                    checkbox.ID = "chkDiasCancelar_" & Format(fecha, "yyyyMMdd") & "_" & Convert.ToInt32(dias * 10)
                    checkbox.Text = Format(fecha, "dd/MM/yyyy") & " (" & Format(dias, "0.0") & " " & TranslateLocale.text("días") & ")"

                    li.Controls.Add(checkbox)
                    ul.Controls.Add(li)
                Next
                phFechasCancelar.Controls.Add(ul)
            Else
                phFechasCancelar.Controls.Add(New LiteralControl(TranslateLocale.text("* Selecciona una solicitud")))
            End If
