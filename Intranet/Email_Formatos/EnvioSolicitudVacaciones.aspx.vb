...
Public Class EnvioSolicitudVacaciones
    ...
    Private Sub CargaFormulario()
        ...
        If dt.Rows.Count > 0 Then
            ...
            If Me.lblAuth.Text = "1" Then
                lblTitulo.Text = TranslateLocale.text("Se ha autorizado la siguiente solicitud de vacaciones:", EMAIL_LOCALE)
            ...
            ElseIf Me.lblAuth.Text = "0" Then
                lblTitulo.Text = TranslateLocale.text("Se ha rechazado la siguiente solicitud de vacaciones:", EMAIL_LOCALE)
                lblNombre.Text = TranslateLocale.text("Estimado(a)", EMAIL_LOCALE) & "&nbsp;" & dr("empleado_solicita").ToString()
            ...
            Else
                lblTitulo.Text = TranslateLocale.text("Se ha generado una solicitud de vacaciones con la siguiente información:", EMAIL_LOCALE)
            ...
            If Me.lblAuth.Text = "" Then
                lnkDetalle.HRef = url_base & "SolicitudVacaciones.aspx"
                Dim valores As String = "&id=" & Request.QueryString("id") & "&ida=" & dr("id_autoriza_jefe")
                lnkAutoriza.HRef = url_base & "SolicitudVacacionesAutoriza.aspx?auth=1" & valores
                lnkRechaza.HRef = url_base & "SolicitudVacacionesAutoriza.aspx?auth=0" & valores
